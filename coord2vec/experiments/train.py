...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_steps', nargs='+', default=[30_000, 50_000], help='List of steps in which we apply an LR step')
    ...
    coord2vec = Coord2Vec(get_builder(), n_channels=3, tb_dir='build_park_road_multi_50_000_long_steps',
                          embedding_dim=opt.embedding_size, lr=opt.start_lr, lr_steps=opt.lr_steps, lr_gamma=opt.lr_gamma)
    coord2vec.fit(train_dataset, val_dataset, epochs=opt.nepoch, batch_size=opt.batch_size, evaluate_every=opt.val_interval)