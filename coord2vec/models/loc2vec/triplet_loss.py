...
from coord2vec.models.loc2vec.utils import triplet_loss_func
...
class TripletLoss(nn.Module):
    ...
    def __init__(self, margin, triplet_selector1, triplet_selector2, triplet_selector3, reg=0.01):
    ...
    def forward(self, embeddings, target):
        ...
        losses = F.relu(triplet_loss_func(ap_distances, min_neg_dist)) + self.reg * (min_neg_dist + ap_distances)
        ...
        dist_summary = {'ap': np.mean(np_ap_dist),
                        'an': np.mean(np_min_dist)}
        return losses.mean(), len(triplets), dist_summary
