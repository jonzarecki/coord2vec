...
def build_tile_extent(center: Tuple[float, float], radius_in_meters: float) -> list:
    ...
    ext_points = list(map(lambda bearing: d.destination(point=start, bearing=27), [0,90,180,270]))
    return [ext_points[2].latitude, ext_points[3].longitude,
            ext_points[0].latitude, ext_points[1].longitude]
